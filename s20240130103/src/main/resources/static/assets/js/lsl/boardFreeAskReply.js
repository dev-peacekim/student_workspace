...
function updateReplyList(data) {
    ...
    $.each(data, function(index, lslCommReply) {
        ...
        // 대댓글 입력창의 ID를 부모 댓글 번호에 기반하여 유일하게 만듭니다.
        const addCommentId = `addComment_${lslCommReply.creply_no}`;
        ...
        boardReplyList += `<div class="re-comment-body">
            <div id="replyBoardFreeList" class="comment-card">
                ...
                <!-- 대댓글 입력창 -->
                <div id="${addCommentId}" class="card-body comment-body" style="display: none;">
                    <textarea id="addCommentText_${lslCommReply.creply_no}" class="form-control"></textarea>
                    <button type="button" class="btn addReply" onclick="addReply('${lslCommReply.creply_no}');">저장</button>
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            if (data > 0) {
                ...
                replyBoardFreeAskList(reReplyData.cboard_no); // 수정: reReplyData.cboard_no로 변경
            }
        },
        error: function(error) {
